...
### 构建python基础镜像
```bash
docker build -t python:3.10 --target python-3.10 .
```

### 构建升腾镜像
```bash
docker build -t ascend:8.1.RC1 --target ascend .
```

### 构建pytorch镜像
```bash
docker build -t pytorch:2.5.1 --target pytorch .
```

### 构建ma-pytorch镜像
```bash
docker build -t ma-pytorch:8.5.1 --target ma-pytorch .
```
...
1. minimind
```bash
docker build -t minimind:v2 --target minimind .
```
2. yolo11
```bash
docker build -t yolo:11 --target yolo .
```
...
### toolkit 中的组件
CANN-runtime-7.7.0.1.238-linux.aarch64.run
CANN-compiler-7.7.0.1.238-linux.aarch64.run
CANN-hccl-7.7.0.1.238-linux.aarch64.run
CANN-opp-7.7.0.1.238-linux.aarch64.run
Ascend-pyACL_8.1.RC1_linux-aarch64.run
Ascend-test-ops_8.1.RC1_linux.run

CANN-toolkit-7.7.0.1.238-linux.aarch64.run
CANN-aoe-7.7.0.1.238-linux.aarch64.run
CANN-ncs-7.7.0.1.238-linux.aarch64.run
...
### kernels组件[required]
Ascend910B-opp_kernel-7.7.0.1.238-linux.aarch64.run

### nnal组件[optional]

> LLM场景下可选。
Ascend-cann-atb_8.1.RC1_linux-aarch64.run
